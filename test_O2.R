...
res<-CalculateIndicator("CoastOxygen",filter(df,year>2009),RefCond_sali,variance_list,MonthInclude,2007,2012,n_iter=1000)

Value<-res$indicator_sim
dfres<-as.data.frame(Value)
ggplot(dfres,aes(Value)) + geom_histogram(binwidth=0.01)
...
df <- mutate(df,xvar=O2)
res<-CalculateIndicator("CoastOxygen",df,RefCond_sali,variance_list,MonthInclude,2007,2012,n_iter=1000)
